  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg p-5 max-w-sm mx-auto text-gray-800 font-sans">
        ...
        <tbody>
          ...
          <tr className="border-b border-gray-200">
            <td className="px-3 py-2 text-sm">
              Leave: {record.leave_days_taken ?? 0}
              {(record.leave_adjustment ?? 0) !== 0 && ` (Adj: ${(record.leave_adjustment ?? 0) > 0 ? '+' : ''}${record.leave_adjustment})`}
              {` (Less from Yearly: ${emp.yearly_leave_allowance})`}
            </td>
      ...
      <div className="flex justify-between mt-4 px-1 text-xs text-gray-600">
        <span>Salary generated by: {settings?.generated_by ?? 'Nahid'}</span>
