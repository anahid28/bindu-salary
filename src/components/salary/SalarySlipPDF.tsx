...
// ── Single slip ─────────────────────────────────────────
function Slip({ calc, month, year, generatedBy }: {
  calc: SalaryCalc
  month: number
  year: number
  generatedBy: string
  ...
  return (
    <View style={s.slip}>
      ...
      <View style={s.table}>
        ...
        <View style={s.tRow}>
          <Cell style={s.tDesc}>
            {`Leave: ${record.leave_days_taken ?? 0} (Less from Yearly Leave: ${emp.yearly_leave_allowance})`}
          </Cell>
    ...
    <Document>
      {pages.map((pair, pi) => (
        <Page key={pi} size="A4" orientation="landscape" style={s.page}>
          <Slip calc={pair[0]} month={month} year={year} generatedBy={generatedBy} />
          {pair[1]
            ? <Slip calc={pair[1]} month={month} year={year} generatedBy={generatedBy} />
            : <View style={{ flex: 1 }} />
          }
        </Page>
      ))}
