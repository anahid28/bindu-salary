    ...
    <div className="bg-white border border-gray-200 rounded-lg p-5 max-w-sm mx-auto text-gray-800 font-sans">
        ...
        <tbody>
          ...
          <tr className="border-b border-gray-200">
            <td className="px-3 py-2 text-sm">
              {`Leave: ${record.leave_days_taken ?? 0}, Less from yearly leave ${record.leave_adjustment ?? 0} (Used: ${String(calc.yearly_used_leave ?? 0).padStart(2, '0')}/ Left: ${String(emp.yearly_leave_allowance - (calc.yearly_used_leave ?? 0)).padStart(2, '0')})`}
            </td>
      ...
      <div className="mt-3 pt-2 border-t border-gray-200">
        <p className="font-mono text-[10px] text-gray-500">
          Salary Generated by Admin ({settings?.generated_by ?? 'Nahid'}), {new Date().toLocaleDateString('en-GB')} {new Date().toLocaleTimeString('en-GB')}
        </p>
      </div>
    </div>
  )
