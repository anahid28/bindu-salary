...
function EidSlip({ calc, title, generatedBy, genTimeStr }: { calc: EidCalc; title: string; generatedBy: string; genTimeStr: string }) {
    ...
    <View style={s.slip}>
      ...
      <View style={{ borderTop: `1 solid ${BORDER}`, marginTop: 10, paddingTop: 5 }}>
        <Text style={{ fontFamily: 'Courier', fontSize: 7, color: '#666' }}>
          Salary Generated by Admin ({generatedBy}), {genTimeStr}
        </Text>
      </View>
    </View>
  )
    ...
    <Document>
      {pages.map((pair, pi) => (
        <Page key={pi} size="A4" orientation="landscape" style={s.page}>
          <EidSlip calc={pair[0]} title={title} generatedBy={generatedBy} genTimeStr={genTimeStr} />
          {pair[1]
            ? <EidSlip calc={pair[1]} title={title} generatedBy={generatedBy} genTimeStr={genTimeStr} />
            : <View style={{ flex: 1 }} />
          }
        </Page>
      ))}
